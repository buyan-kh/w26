...
export class PromptRegistryMCP {
    ...
    async getPrompt(args) {
        ...
        if (package_name) {
            const packageData = this.packages.get(package_name);
            if (packageData) {
                foundPrompt = packageData.prompts.find((p) => p.id === prompt_id && (!version || p.version === version));
            }
        }
        else {
            for (const packageData of this.packages.values()) {
        ...
        if (!foundPrompt) {
            throw new Error(`Prompt '${prompt_id}' not found${version ? ` (version ${version})` : ""}`);
        }
        return {
            content: [
                {
    ...
    async searchPrompts(args) {
        ...
        for (const packageData of this.packages.values()) {
            for (const prompt of packageData.prompts) {
                ...
                const matchesTags = !tags || tags.some((tag) => prompt.tags.includes(tag));
    ...
    calculateConfidence(original, processed) {
        ...
        const intersection = new Set([...originalChars].filter((x) => processedChars.has(x)));
    ...
    async gradePromptExecution(args) {
        const { prompt_id, user_input, ai_response, expected_outcome, grader_type = "rule_based", } = args;
        let grade;
        if (grader_type === "llm" && this.openai) {
            grade = await this.gradeWithLLM(prompt_id, user_input, ai_response, expected_outcome);
        }
        else {
            grade = this.gradeWithRules(prompt_id, user_input, ai_response, expected_outcome);
        }
        // Update analytics
    ...
    async gradeWithLLM(prompt_id, user_input, ai_response, expected_outcome) {
        if (!this.openai) {
...
`;
    ...
    gradeWithRules(prompt_id, user_input, ai_response, expected_outcome) {
    ...
    scoreAccuracy(user_input, ai_response) {
        let score = 5.0;
        ...
        if (errorIndicators.some((indicator) => ai_response.toLowerCase().includes(indicator))) {
            score -= 2.0;
        }
    ...
    scoreHelpfulness(user_input, ai_response) {
        let score = 5.0;
    ...
    scoreCompleteness(user_input, ai_response) {
        let score = 5.0;
        if (ai_response.length > user_input.length * 2) {
            score += 2.0;
        }
        if (ai_response.split("\n").length > 2 ||
            ai_response.split(".").length > 3) {
            score += 1.0;
        }
        return Math.max(1.0, Math.min(10.0, score));
    }
    scoreClarity(ai_response) {
        let score = 5.0;
    ...
    async getBestPrompt(args) {
        ...
        for (const packageData of this.packages.values()) {
            for (const prompt of packageData.prompts) {
                ...
                else if (inputLower.includes("customer") &&
                    promptTags.includes("customer")) {
                    bestPrompt = prompt;
                    break;
                }
                else if (inputLower.includes("explain") &&
...
//# sourceMappingURL=prompt-registry-mcp.js.map